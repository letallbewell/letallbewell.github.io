...
\begin{itemize}
\item
uses action principle in terms of displacement variables
\item
...
= (\mathbf{F}^\top \mathbf{F}) \cdots (\mathbf{F}^\top \mathbf{F}) - \mathbf{I}, p \neq 0$ and $\ln \mathbf{F}^\top \mathbf{F}$
\\

physically, $\mathbf{F}^\top \mathbf{F}$ gives change in length of a material element, 
...
where 
$\lambda = |\vec{r}_x|-1$ is the strain (net stretch)
\\
...
\quad
$k_0 = \const\neq 0$\\
In this case, there are 14 symmetries, of which 13 are variational symmetries. The uniform scaling symmetry is not variational.
